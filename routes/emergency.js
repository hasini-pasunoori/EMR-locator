...
router.post('/:id/respond', async (req, res) => {
  try {
    ...
    // Create chat between requester and responder
    let chat = await Chat.findOne({
      participants: { $all: [request.requester, req.user._id] },
    ...
    if (!chat) {
      ...
      const initialMessage = {
        sender: req.user._id,
        message: `Hi! I'm responding to your ${request.type} emergency request. ${req.body.message || ''}`,
        timestamp: new Date()
      };
...
router.get('/user/outgoing-responses', async (req, res) => {
  try {
    ...
    const requestsWithMyResponse = requests.map(request => {
      const myResponse = request.responses.find(r => r.responder._id.toString() === req.user._id.toString());
      return {
        ...request.toObject(),
        myResponse
      };
...
router.get('/stats/overview', async (req, res) => {
  try {
    const stats = await Promise.all([
      EmergencyRequest.countDocuments({ status: 'active', $or: [{ isActive: true }, { isActive: { $exists: false } }] }),
      EmergencyRequest.countDocuments({ status: 'fulfilled' }),
      EmergencyRequest.countDocuments({ urgency: 'critical', status: 'active' }),
      BloodDonor.countDocuments({ 'availability.isAvailable': true, $or: [{ isActive: true }, { isActive: { $exists: false } }] }),
      MedicalFacility.countDocuments({ $or: [{ isActive: true }, { isActive: { $exists: false } }] })
    ]);
